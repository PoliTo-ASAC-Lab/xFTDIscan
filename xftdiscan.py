...
parser = argparse.ArgumentParser(
    description="xFTDIscan - Cross Platform FTDI Device Scanner."
)
parser.add_argument(
    "-v",
    "--verbose",
    action="store_true",
    help="If present, more info about found devices will be printed.",
)
parser.add_argument(
    "-l",
    "--list",
    action="store_true",
    help="If present, prints device list also as simple CSV list.",
)
...
def win_usbscan():
    ...
    dev_l = []
    device_scan_command = (
        'powershell pnputil /enum-devices /connected  /class "Ports"'.split()
    )
    dev_scan_proc = subprocess.Popen(
        device_scan_command,
        shell=True,
        stdout=subprocess.PIPE,
    )
    dev_scan_raw_output = (
        dev_scan_proc.stdout.read()
    ...
    for n, dev_str in enumerate(dev_str_l):
        ...
        if any(x in dev_str for x in ["ftdi", "FTDI", "0403"]):
            ...
            dev_name = (
                field_str_l[0].split(":")[-1].strip()
                if "ID" in field_str_l[0]
                else "???"
            )  # Instance ID
            com_port = (
                field_str_l[1].strip().split(":")[-1].split("(")[-1][:-1]
                if "Des" in field_str_l[1]
                else "???"
            )  # Device Description
...
def linux_usbscan():
    ...
    """
    device_scan_command = shlex.split(
        " ".join(" ".join(device_scan_script.splitlines()).split())
    )
...
if __name__ == "__main__":
    if verbose:
        print(f"System type is {platform.system()}")
    if platform.system() == "Windows":
        print_dev_l(win_usbscan())
    elif platform.system() == "Linux":
        print_dev_l(linux_usbscan())
